...
class BhimParser : PaymentNotificationParser {
    override val provider: Provider = Provider.BHIM
    ...
    override fun parse(notification: NormalizedNotification): ParseResult {
        ...
        if (direction != Direction.CREDIT) {
            return ParseResult.Ignored("BHIM notification is not an incoming credit")
        }

        val amountMinor = AmountParser.extractAmountMinor(text)
            ?: return ParseResult.Failed("Could not extract amount from BHIM notification")
